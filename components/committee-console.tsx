...
export function CommitteeConsole(){
  const [form,setForm]=useState(initial); const [result,setResult]=useState<Recommendation|null>(null); const [messages,setMessages]=useState<SessionMessage[]>([]); const [loading,setLoading]=useState(false); const [error,setError]=useState(""); const [activeMember,setActiveMember]=useState<MemberKey|"">(""); const [voiceEnabled,setVoiceEnabled]=useState(false);
  ...
  async function speak(message:SessionMessage){
    if(!voiceEnabled)return delay(1050);
  ...
  async function revealSession(recommendation:Recommendation){
    ...
    for(const opinion of recommendation.opinions){ const key=memberKeyFromId(opinion.memberId); const member=members.find(item=>item.key===key)!; await showMessage({id:opinion.memberId,role:member.role,initials:member.initials,status:"speaking",body:opinion.thesis,vote:voteText[form.language][opinion.vote]}); }
  ...
  function resetSession(){audioRef.current?.pause();if(typeof window!=="undefined"&&"speechSynthesis"in window)window.speechSynthesis.cancel();setResult(null);setMessages([]);setActiveMember("");roomBodyRef.current?.scrollTo({top:0,behavior:"smooth"});}
